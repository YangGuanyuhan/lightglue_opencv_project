...
const int DESCRIPTOR_DIM = 128;  // ALIKED uses 128-D descriptors
...
{
    cout << "Running ALIKED+LightGlue inference (fused format)" << endl;
    ...
    // Fused model: only matches0 [M,2] int64 and mscores0 [M] float
    ...
    for (int i = 0; i < 10 && i < M; i++)
    {
        ...
        if (is_int64)
        {
            const int64_t* md = (const int64_t*)matches0.ptr<int64_t>(0);
...
void test_model(const string &model_path)
{
    ...
    catch (const cv::Exception &e)
    {
        ...
        cerr << "Hint: ENGINE_NEW may not support this model. Try test_aliked_lightglue_ort with ENGINE_ORT." << endl;
    }
...
int main()
{
    ...
    test_model("../model/aliked_lightglue.onnx");
